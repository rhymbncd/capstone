...
<head>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .bottom-link { text-align: center; font-size: 13px; color: #888; margin-top: 20px; }
...
<div class="card">
  ...
  <p class="sub">Enter the email address for your {{ $portalType ?? 'student' }} account and we'll send a password reset link to your inbox.</p>
  ...
  <form method="POST" action="{{ route(($portalType ?? 'student').'.password.email') }}">
    @csrf
    <div class="ig">
      <label>Email address</label>
      <div class="iw">
        <i class="fa-solid fa-envelope ico"></i>
        <input type="email" name="email" placeholder="Enter your email" autocomplete="email" required value="{{ old('email') }}">
      </div>
  ...
  <p class="bottom-link">Remembered it? <a href="{{ route(($portalType ?? 'student').'.login') }}">Sign in</a></p>
</div>
</body>
</html>
